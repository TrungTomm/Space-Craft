...
export default class linkedScene extends Phaser.Scene {
    constructor() {
        super('linkedScene');
    }

    initialize() {
        Phaser.Scene.call(this, { key: 'linkedScene' });
    }
    ...
    popultedScene() {
        ...
        this.money = this.add.text(this.walBlue.x - 20, this.walBlue.y + width * (40 / 430), '200000', {
            fontFamily: 'Lexend',
            fontSize: '45px',
            color: '#B3F8FF',
            fontStyle: 'bold'
        ...
        this.moneyIcon = this.add
            .image(this.money.x + 85, this.money.y, 'moneyIcon')
            .setOrigin(0, 0.5);

        this.lameBg = this.add
            .nineslice(this.walBlue.x, this.money.y + width * (25 / 430), 'lameBg', 0, width * (220 / 430), 40, 10, 10, 10, 10)
            .setOrigin(0.5, 0);

        this.token = this.add.text(this.lameBg.x, this.lameBg.y + 9, 'XXBFu4849292848', {
            fontFamily: 'Lexend',
            fontSize: '20px',
        })
            .setOrigin(0.5, 0);

        this.copyIcon = this.add
            .image(this.lameBg.x + width * (130 / 430), this.lameBg.y + 5, 'copy')
            .setOrigin(0.5, 0);

        this.container2.add([this.iconBlue, this.walBlue, this.playerName2, this.receiveText, this.sendText, this.disconnectText, this.receive, this.send, this.disconnect, this.money, this.moneyIcon, this.lameBg, this.token, this.copyIcon]);
        this.container2.setAlpha(1);
    }
